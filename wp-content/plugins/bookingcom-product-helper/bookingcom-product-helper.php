 ...
 */
define( 'BOOKINGCOM_PRODUCT_HELPER_VERSION', '1.0.0' );
...
if ( is_admin() ) {
	...
	function bcph_add_action_links( $links ) {
		...
		$settings_link .= __( 'Settings', 'bookingcom-product-helper' );
		$settings_link .= '</a>';
...
function bcph_load_plugin_settings() {
	...
	if (
		...
		wp_verify_nonce( sanitize_key( $_GET['bookingcom_product_helper_nonce'] ), 'remove_widget_helper' )
	) {
		delete_option(
			wp_unslash(
				sanitize_key( 'booking_product_helper_shortname-' . $_GET['remove_widget_helper'] )
			)
		);
		...
		if ( is_array( $bookingcom_product_helper_list ) &&
			in_array(
				wp_unslash( $_GET['remove_widget_helper'] ),
			...
			$bookingcom_product_helper_list = array_diff(
				...
				array(
					wp_unslash(
						sanitize_key( $_GET['remove_widget_helper'] )
					),
				)
			);
...
function bcph_edit_widget() {
	...
	if (
		...
		$bdotcom_widget_helper_id = sanitize_text_field(
			wp_unslash( $_GET['edit_widget_helper'] )
		);
	}
	...
	if (
		...
		foreach ( $_POST as $k => $v ) {
			$bookingcom_product_helper[ $k ] = stripslashes( $v );
		}
				...
				<div class="form-group">
					...
					<div class="form-control disabled">
						...
						if ( isset( $bdotcom_widget_helper_id ) ) {
							echo esc_attr( $bdotcom_widget_helper_id );
						}
						?>
					</div>
				</div>
				...
				<div class="form-group">
					...
					<textarea
						...
						cols="50"><?php if ( isset( $bookingcom_product_helper['code_content'] ) ) {
							echo esc_attr( $bookingcom_product_helper['code_content'] );
						} ?></textarea>
				</div>
				...
				<div class="form-group">
					...
					<textarea
						...
						if ( isset( $bookingcom_product_helper['short_description'] ) ) {
							echo esc_attr( $bookingcom_product_helper['short_description'] );
						} ?></textarea>
...
function bcph_add_widget() {
	...
	if (
		...
		if ( count( $error_messages ) <= 0 ) {
			$bdotcom_widget_helper_id         = strtolower( $bookingcom_product_helper['bdotcom_widget_helper_id'] );
						...
						<textarea
							...
							cols="50"><?php
								if ( isset( $bookingcom_product_helper['bdotcom_widget_code'] ) ) {
									echo esc_attr( $bookingcom_product_helper['bdotcom_widget_code'] );
								} ?></textarea>
					</div>
